...
async def get_resource_timeline(
    ...
    # ---- Step 4: 自动检测资源类型 ----
    # WCL resourceChangeType ID 在不同扩展/职业间可能不同
    # "auto" 模式: 选择出现最多的 resourceChangeType
    from collections import Counter
    type_counts = Counter(e.get("resourceChangeType") for e in events if e.get("type") == "resourcechange")
    ...
    if resource_type_lower == "auto":
        ...
        # 尝试匹配 — 由于 WCL type ID 不稳定，直接用最常见的
        # 大多数职业只有一种主要资源类型
        target_type_id = type_counts.most_common(1)[0][0]
        detected_name = resource_type_lower

    # ---- Step 5: 提取资源值 ----
    # WCL resourcechange 事件只记录资源变化量（delta），不记录当前绝对值。
    # 显示每次变化: gain（获取量）、waste（溢出量）、max。
    ...
    for evt in events:
        ...
        points.append(ResourcePoint(
            ...
            value=change,       # 资源变化量（正=获取）
            max_value=max_val,  # 资源上限
            spell_name=spell_name,
            is_overflow=is_overflow,
        ))

    total_points = len(points)
    overflow_pct = round((overflow_count / total_points * 100.0), 1) if total_points > 0 else 0.0

    return ResourceTimelineResponse(
        report_code=report_code,
        fight_id=fight_id,
        player_name=player,
        resource_type=detected_name,
        fight_duration=round(fight_duration, 1),
