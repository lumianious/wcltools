...
def check_player_apl(
    ...
    # 构建 WCL spell_id → SimC spell name 反向映射
    wcl_to_simc: dict[int, str] = {v: k for k, v in simc_to_wcl.items()}
    ...
    # 识别 DoT/维护类技能 — 含 target_if 条件且无 CD，无法评估优先级
    # DoT/维护类和 Eclipse 进入技能 — 含 target_if 或不可直接施放，无法评估优先级
    _DOT_SPELLS = {"moonfire", "sunfire", "stellar_flare",
                    "solar_eclipse", "lunar_eclipse"}
    dot_spell_ids: set[int] = set()
    for simc_spell in _DOT_SPELLS:
        wcl_id = simc_to_wcl.get(simc_spell)
        if wcl_id:
            dot_spell_ids.add(wcl_id)

    # 构建 APL 规则的 spell_id 列表（按优先级排序）
    # 排除 DoT/维护技能 — 它们有 target_if 条件，我们无法评估
    apl_spell_priority: list[tuple[str, int, int]] = []  # (simc_spell, wcl_id, priority)
    for rule in rules:
        simc_spell = rule.get("spell", "")
        wcl_id = simc_to_wcl.get(simc_spell)
        if wcl_id and wcl_id not in dot_spell_ids:
            # 去重: 只保留每个技能的最高优先级规则
            if not any(s == simc_spell for s, _, _ in apl_spell_priority):
                apl_spell_priority.append((
    ...
    # 计算合规率
    total_tracked_casts = sum(
        1 for _, sid in sorted_casts if sid in wcl_to_simc
    )
    violation_count = len(violations)
    compliance_pct = (
        round((1 - violation_count / total_tracked_casts) * 100, 1)
        if total_tracked_casts > 0 else 100.0
    )
    ...
    top_patterns = [
        f"{pattern} ({count}x)"
        for pattern, count in violation_patterns.most_common(5)
    ]
